...
def generate_css(css_links):
    return '\n'.join(
        ['<link rel="stylesheet" href="{}">'.format(l) for l in css_links]
    )


def generate_js(js_links):
    return '\n'.join(['<script src="{}"></script>'.format(l) for l in js_links])
...
class IntegrationTest(unittest.TestCase):
    ...
    def test_dependencies_route(self):
        ...
        self.assertEqual(
            json.loads(response.data),
            {'header': {'state': [{'id': 'id1'}], 'events': [{'id': 'id1'}]}},
        )
        ...
        self.assertEqual(
            json.loads(response.data),
            {'header': {'state': state, 'events': events}},
        )
            ...
            data=json.dumps(
                {
                    ...
                    'target': {'props': {'id': 'header'}},
                    'parents': {'id1': {'props': {'value': 'New Value'}}},
                }
            ),
        )
    ...
    def test_single_observer_returning_a_component(self):
        ...
        def update_header(input1):
            self.assertEqual({'value': 'New Value'}, input1)
            return {'children': Div('New Component')}
    ...
    def test_single_observer_updating_component_that_doesnt_exist(self):
        ...
        self.assertEqual(
            ...
            {
                'response': {
                    'props': {'id': 'doesnt-exit-yet', 'value': 'New Value'}
                }
            },
        )
    ...
    def test_single_observer_with_multiple_controllers(self):
        ...
        self.assertEqual(
            json.loads(response.data),
            {'response': {'props': {'id': 'header', 'value': 'New Value'}}},
        )
    ...
    def test_serving_scripts(self):
        ...
        response = self.client.get(
            ('/_dash-component-suites/' 'dash_renderer/bundle.js?v={}').format(
                dash_renderer.__version__
            )
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.data, pkgutil.get_data('dash_renderer', 'bundle.js')
        )
...
class TestCallbacks(unittest.TestCase):
    def test_callback_registry(self):
        ...
        app.layout = Div(
            [input, Div(id='output-1'), Div(id='output-2'), Div(id='output-3')],
            id='body',
        )
    ...
    def test_exception_event_not_in_component(self):
        ...
        for id in ['output', 'body']:
            ...
            app.callback(Output(id, 'children'), events=[Event(id, 'click')])
        ...
        app.callback(
            Output('graph-output', 'children'), events=[Event('graph', 'click')]
        )
    ...
    def test_exception_component_is_not_right_type(self):
        ...
        for args in test_args:
            self.assertRaises(
                exceptions.IncorrectTypeException, app.callback, *args
            )
    ...
    def test_suppress_callback_exception(self):
        ...
        app.callback(
            Output('id-not-there', 'children'), [Input('input', 'value')]
        )

    def test_missing_input_and_events(self):
        app = dash.Dash('')
        app.layout = Div([dcc.Input(id='input')], id='body')
        self.assertRaises(
